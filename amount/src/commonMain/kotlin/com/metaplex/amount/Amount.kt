 ...
 * @category Utils — Amounts
 */
fun lamports(lamports: BigInteger): SolAmount = createAmount(lamports, "SOL", 9)
fun lamports(lamports: Int): SolAmount = createAmount(BigInteger(lamports), "SOL", 9)
fun lamports(lamports: Long): SolAmount = createAmount(BigInteger(lamports), "SOL", 9)
...
fun <I : AmountIdentifier, D : AmountDecimals> isEqualToAmount(
    ...
    val delta = absoluteAmount(subtractAmounts(left, right))
    val toleranceOrDefault = tolerance ?: createAmount(BigInteger.ZERO, left.identifier, left.decimals)
